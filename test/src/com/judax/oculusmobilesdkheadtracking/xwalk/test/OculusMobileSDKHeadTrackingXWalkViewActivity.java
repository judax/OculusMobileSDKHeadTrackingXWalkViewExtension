...
public class OculusMobileSDKHeadTrackingXWalkViewActivity extends Activity
{
	...
	private class CrosswalkResourceClientInjectJSCode extends XWalkResourceClient
	{
		...
		public CrosswalkResourceClientInjectJSCode(XWalkView xwalkView, String url,
				String jsCode)
		{
			super(xwalkView);
			this.url = url;
	...
	public class ReadFromURLAsyncTask extends AsyncTask<Void, Void, Void>
	{
		...
		public ReadFromURLAsyncTask(String url,
				ReadFromURLAsyncTaskListener listener)
		{
			this.url = url;
		...
		protected Void doInBackground(Void... params)
		{
			...
			{
				System.err.println("JUDAX: MalformedURLException while reading the '"
						+ url + "' file.");
				e.printStackTrace();
			...
			{
				System.err.println("JUDAX: IOException while reading the '" + url
						+ "' file.");
				e.printStackTrace();
				listener.exception(this, e);
			}
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		new ReadFromURLAsyncTask(
				"https://raw.githubusercontent.com/judax/OculusMobileSDKHeadTrackingWebVR/master/js/OculusMobileSDKHeadTrackingWebVR.js",
				new ReadFromURLAsyncTaskListener()
				{
					@Override
					public void success(ReadFromURLAsyncTask source, final String jsCode)
					{
						runOnUiThread(new Runnable()
						{
							...
							public void run()
							{
								...
								{
									crosswalkView
											.setResourceClient(new CrosswalkResourceClientInjectJSCode(
													crosswalkView, url, jsCode));
								}
					...
					@Override
					public void exception(final ReadFromURLAsyncTask source,
							final Exception e)
					{
						runOnUiThread(new Runnable()
						{
							...
							public void run()
							{
								...
								{
									message = "Error while loading the JS extension file from the given URL '"
											+ source.getUrl() + "'.";
								}
								else if (e instanceof MalformedURLException)
								{
									message = "The given URL '" + source.getUrl()
											+ "' is not correct.";
								}
								AlertDialog alertDialog = createAlertDialog(
										OculusMobileSDKHeadTrackingXWalkViewActivity.this,
										"Error loading JS extension file",
										message
												+ " The WebVR API polyfill won't be present. Load the page anyway?",
